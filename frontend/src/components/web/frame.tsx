...
export default function WebFrame(Component: any) {
    ...
    return function webFrame(props: any){
        ...
        useEffect(() => {
            ...
            // Monitor class changes on the body or the desired parent
            ...
            // Initial call in case the class is already applied
            applyStyles();
                    ...
                    <>
                        ...
                        <nav id={'pageNav'} className={`flex flex-wrap lg:flex-nowrap sm:h-36 h-16 pr-5 md:pr-36 w-full fixed top-0 items-center justify-center transition-all duration-500 z-50 uppercase  ${isAtTop ? 'bg-transparent text-white' : 'bg-white text-black shadow-lg'}`}>
                            ...
                            <div className="flex-grow"></div>
                            {
                                pages.map((page: any) => {
                                    return (
                                            <a href={`/${page.url}`} className="hidden lg:block px-3 flex-shrink mx-1.5 text-base hover:text-[#58585a] transition-all duration-350 tracking-navNormal">
                                                <LangConv en={page.display_name} tr={page.display_name_tr} />
                                            </a>
                                        )
                                })
                            }
                            <a href="/bize-ulasin" className="hidden lg:block px-3 flex-shrink mx-1.5 text-base hover:text-[#58585a] transition-all duration-350 tracking-navNormal">
                                <LangConv en={'contact us'} tr={'bize ulaşın'} />
                            </a>
                            <span onClick={()=>{dispatch(switchLanguage() as unknown as UnknownAction)}} className="cursor-pointer hidden lg:block px-3 mx-1.5 text-base tracking-navNormal">
                            ...
                            >
                                ...
                                <li>
                                    <button onClick={()=>{openSearchModal(); setIsNavOpen(!isNavOpen)}} className={"text-black !filter-none cursor-pointer"}>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div>
                            ...
                            {/*{!--*/}
                                ...
                                {/*    To: "opacity-0"*/}
                                {/*    --}*/}
                                <div className={`fixed inset-0 bg-gray-500/75 transition-opacity ${isSearchModalOpen ? ' opacity-100 ease-out duration-300 w-full' : 'ease-in duration-200 opacity-0 w-0' }`}
                                     aria-hidden="true"></div>

                                <div className={`fixed inset-0 z-10 overflow-y-auto ${isSearchModalOpen ? ' w-screen' : ' w-0'}`}>
                                    <div
                                        ...
                                        <div onClick={(e)=>{e.stopPropagation()}}
                                            className={`relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg ${isSearchModalOpen ? ' ease-out duration-300 opacity-100 translate-y-0 sm:scale-100' : 'ease-in duration-200 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95'}`}>
                                            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 w-full">
                                                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                        <span className={"float-right zoomIn text-base font-semibold text-gray-900 cursor-pointer"}  onClick={closeSearchModal}>x</span>
                                                        ...
                                                        <hr/>
                                                        <div className="mt-2">
                                                            <input type="search" className="w-full p-3 border rounded text-black" name="search" value={searchString} onChange={(e)=>{setSearchString(e.target.value)}}
